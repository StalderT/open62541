...
/* Coverage for the per-node MonitoredItem backpointer list (samplingInterval==0
 * "on write" sampling). Exercises O(1) unlink (head/interior/tail), the on-write
 * iterator, and the copy-on-write duplication of a Node that carries attached
 * MonitoredItems. */
...
/* Walk the node's backpointer list, assert the doubly-linked invariant
 * (every element's prev points to its predecessor) and return the length. */
static size_t
nodeBackpointerCount(UA_NodeId nodeId) {
    lockServer(server);
    const UA_Node *n = UA_NODESTORE_GET(server, &nodeId);
    ck_assert_ptr_ne((void*)n, NULL);
    size_t cnt = 0;
    UA_MonitoredItem *prev = NULL;
    for(UA_MonitoredItem *m = n->head.monitoredItems; m;
        m = m->sampling.nodeList.next) {
        ck_assert_ptr_eq(m->sampling.nodeList.prev, prev);
        prev = m;
        cnt++;
    }
...
START_TEST(Server_monitoredItems_sameNode_backpointer) {
     ...
     * they all land on the per-node "on write" backpointer list. */
    const size_t K = 5;
    UA_UInt32 ids[5];
    UA_MonitoredItemCreateRequest item;
    UA_MonitoredItemCreateRequest_init(&item);
    ...
    /* Write the node: triggers the on-write iterator over the list AND the
     * copy-on-write duplication of the Node together with its MonitoredItem
     * list. The list must survive intact. */
    UA_Int32 nv = 2;
    UA_Variant v;
